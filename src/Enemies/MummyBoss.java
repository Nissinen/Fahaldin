...
import Projectiles.MummyEye;
import TileMap.TileMap;

public class MummyBoss extends Enemy {
	...
	//basic monster stuff
		...
		public MummyBoss(TileMap tm, Player p, ArrayList<Enemy> en) {
			super(tm);
			health = maxHealth = 30;
			// monster health bar
			try{
			...
			walkSprites = Content.Mummy[0];
			attackSprites = Content.Mummy[1];
		...
		private void setAction() {
			...
			if(action == 0) {
					...
					if(action == 1) {
						...
						if(currentAction == ATTACKING && firedOnce == false && animation.getFrame() >= 12){
							firedOnce = true;
								generateEye();
						}
						if(currentAction == ATTACKING && animation.getFrame() >= 13) {
							moveSpeed = 1.5;
							attacking = false;
							action++;
						}
		...
		private void generateEye(){

				me = new MummyEye(tileMap, player);
				me.setPosition(x, y);
				enemies.add(me);
		...
		public void update() {
			...
			calculateCorners(x, ydest - 5, "blocked");
			if(bottomLeft) {
				left = false;
		...
		public void draw(Graphics2D g){
			...
			for(int i = 0; i < health; i++) {
				g.drawImage(hpBar,hpBarX + i * hpBarDistance + 8,hpBarY - cheight / 2 , null );
				}
